 ...
 public:
  [[maybe_unused]] TmpDir() {
    try {
      ...
      const auto called_from_ci = std::getenv("MODLE_CI") != nullptr;  // NOLINT(*-mt-unsafe)
      if (!called_from_ci) {
        throw;
      }
  ...
  [[nodiscard]] static std::string generate_random_file_name(std::mt19937_64& rand_eng,
                                                             std::string_view prefix = "modle-tmp-",
                                                             std::size_t size = 16) {
    assert(size != 0);
    // clang-format off
  ...
  [[nodiscard]] static std::string generate_random_file_name(std::string_view prefix = "modle-tmp-",
                                                             std::size_t size = 16) {
  ...
  [[nodiscard]] static std::filesystem::path create_uniq_temp_dir(
    ...
    return dir;
#else
    auto dir = (tmpdir / "modle-tmp-XXXXXXXXXX").string();
    if (!mkdtemp(dir.data())) {
      throw std::runtime_error(fmt::format(
