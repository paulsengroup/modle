...
constexpr ConstMap<Key, Value, Size>::ConstMap(KeyInputIt first_key, KeyInputIt last_key,
  ...
  const auto dist = static_cast<std::size_t>(std::distance(first_key, last_key));
  if (dist < Size) {
  ...
  std::move(first_value, first_value + static_cast<std::ptrdiff_t>(dist), _vals.begin());
...
template <class Key, class Value, std::size_t Size>
constexpr std::size_t ConstMap<Key, Value, Size>::size() const noexcept {
  return _keys.size();
}
...
constexpr auto ConstMap<Key, Value, Size>::find(const Key &key) const noexcept -> const_iterator {
  ...
  return iterator(*this, static_cast<std::size_t>(std::distance(_keys.begin(), it)));
}
