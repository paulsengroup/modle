...
std::vector<std::size_t> sort_vector_by_idx(const Iterator begin, const Iterator end) {
  std::vector<std::size_t> vi(std::distance(begin, end));
  std::iota(vi.begin(), vi.end(), 0);
  std::sort(vi.begin(), vi.end(),
            [&](std::size_t i1, std::size_t i2) { return *(begin + i1) < *(begin + i2); });
  return vi;
}
...
template <typename Iterator,
          typename = typename std::enable_if<
              std::is_base_of<std::random_access_iterator_tag,
                              typename std::iterator_traits<Iterator>::iterator_category>::value>>
[[maybe_unused]] void sort_pair_of_vectors(const Iterator v1_begin, const Iterator v1_end,
                                           const Iterator v2_begin, const Iterator v2_end) {
...
std::vector<double> compute_element_ranks(const Iterator begin, const Iterator end) {
  ...
  for (auto i = 1U; i < vr.size(); ++i) {
    ...
    if (prev != current) {
      ...
      const auto min = i - 1;
      auto max = i;
      for (auto j = max + 1; j < vr.size() && current == *(begin + vi[j]); ++j) {
        max = j;
      }
