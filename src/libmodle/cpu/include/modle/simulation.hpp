...
class Simulation : Config {
 ...
 public:
  ...
  struct TaskPW : BaseTask {  // NOLINT(altera-struct-pack-align)
    ...
    bp_t range_start{};
    bp_t range_end{};
    absl::Span<const bed::BED> features{};
  };
 ...
 private:
  ...
  //! \p state
  void simulate_window(const bed::BED& reference_feature, StatePW& state, std::ifstream& out_file,
  ...
  void worker(  // Pair of feature + task
      moodycamel::BlockingConcurrentQueue<std::pair<const bed::BED*, Simulation::TaskPW>>& s,
      std::ifstream& out_file, std::mutex& deletion_begin, std::atomic<bool>& end_of_simulation,
