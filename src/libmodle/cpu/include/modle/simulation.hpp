...
class Simulation : Config {
 ...
 private:
  ...
  //! \p state
  void simulate_window(StatePW& state, std::unique_ptr<FILE, decltype(&utils::fclose)>& out_file,
  ...
  void worker(moodycamel::BlockingConcurrentQueue<Simulation::TaskPW>& task_queue,
              std::unique_ptr<FILE, decltype(&utils::fclose)>& out_file, std::mutex& deletion_begin,
