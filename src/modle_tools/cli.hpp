...
class Cli {
 ...
 private:
  ...
  inline void make_eval_subcommand() {
    ...
    sc->add_option("-b,--bin-size", this->_config.bin_size, "Bin size in base pairs. Only used when the contact matrix passed to --reference-matrix is in .mcool format.")->check(CLI::PositiveNumber)->transform(remove_trailing_zero_from_floats);
    ...
    sc->add_option("-w,--diagonal-width", this->_config.diagonal_width, "Diagonal width to use when computing correlation coefficients.")->check(CLI::NonNegativeNumber)->transform(remove_trailing_zero_from_floats)->required();
    sc->add_option("--sliding-window-size", this->_config.sliding_window_size, "Sliding window size. By default this is set to the diagonal width of ModLE's contact matrix.")->check(CLI::NonNegativeNumber)->transform(remove_trailing_zero_from_floats);
    sc->add_option("--sliding-window-overlap", this->_config.sliding_window_overlap, "Overlap between consecutive sliding-windows.")->check(CLI::NonNegativeNumber)->transform(remove_trailing_zero_from_floats)->capture_default_str();
  ...
  inline void make_noisify_subcommand() {
    ...
    sc->add_option("-i,--input", this->_config.path_to_input_matrix, "Path to a contact matrix in Cooler format")->check(CLI::ExistingFile)->required();
    sc->add_option("-o,--output-name", this->_config.path_to_output_matrix, "Output file name (possibly including directories) to use for output.")->required();
    sc->add_option("-w,--diagonal-width", this->_config.diagonal_width, "Diagonal width of the input contact matrix.")->check(CLI::NonNegativeNumber)->transform(remove_trailing_zero_from_floats)->required();
    sc->add_flag("-f,--force", this->_config.force, "Overwrite existing file(s).")->capture_default_str();
  ...
  inline void make_stats_subcommand() {
    ...
    sc->add_option("-w,--diagonal-width", this->_config.diagonal_width, "Diagonal width.")->check(CLI::PositiveNumber)->transform(remove_trailing_zero_from_floats)->required();
