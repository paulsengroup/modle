...
bigwig::file init_bigwig_file(std::string_view output_path,
                              std::vector<std::pair<std::string, I>>& chromosomes, i32 zoom_levels,
                              usize buff_size) {
  static_assert(std::is_integral_v<I>, "I should be an integral numeric type.");
  std::vector<char*> chrom_names(chromosomes.size());
  std::vector<u32> chrom_sizes(chromosomes.size());

  for (usize i = 0; i < chromosomes.size(); ++i) {
    ...
    chrom_sizes[i] = static_cast<u32>(chromosomes[i].second);
  }
...
void write_range(absl::flat_hash_map<std::pair<std::string, N1>, std::vector<N2>>& data, u64 offset,
...
template <typename N1, typename N2>
u64 write_range(absl::flat_hash_map<std::pair<std::string, N1>, std::vector<N2>>& data, u64 offset,
                u64 span, u64 step, std::string_view output_file) {
